...
<div class="launch"><?php 
$another=$_GET[user];
if($_POST['friend']){
	$sql1="SELECT * FROM friend WHERE friend='$_COOKIE[username]' and username='$another' AND username IN (SELECT friend FROM friend WHERE username='$_COOKIE[username]')";
...
<label style="font-size: 22px; font-weight: bold">This is&nbsp;<?php echo $another;?>'s
page</label><br>
<form name="myform6" method="post" action="#"><input type='submit'
	name='friend' value='add friend' /><br>
</form>
</div>
<div class="recent"><label
	style="margin-top: 50px; font-size: 25px; font-weight: bold; color: #0066FF">Most
popular recently</label>
...
while($row1=mysql_fetch_array($querry1)){
	?>
<div style="float: left; width: 350px; height: 250px"><a
	href="picture.php?picture_name=<?=$row1[filedata]?>"><img class="space"
	src="/picture/<?=$row1[filedata]?>" /></a></div>
...
?></div>
<div class="recent"><label
	style="margin-top: 50px; font-size: 25px; font-weight: bold; color: #0066FF">Photo
he posted recently</label>
<hr style="border: 1px dotted #ddd; width: 100%; margin-top: 10px" />
<?php
...
while ($row2=mysql_fetch_array($querry2)){
	?>
<div style="float: left; width: 350px; height: 250px"><a
	href="picture.php?picture_name=<?=$row2[filedata]?>"><img class="space"
	src="/picture/<?=$row2[filedata]?>" /></a></div>
